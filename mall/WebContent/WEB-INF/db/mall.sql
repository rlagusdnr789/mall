...
CREATE TABLE IF NOT EXISTS `p_grade` (
  ...
  `e_pay` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- 내보낼 데이터가 선택되어 있지 않습니다.

-- 테이블 bigdb.score 구조 내보내기
...
-- 테이블 bigdb.scorebase 구조 내보내기
DROP TABLE IF EXISTS `scorebase`;
CREATE TABLE IF NOT EXISTS `scorebase` (
...
-- 테이블 bigdb.student 구조 내보내기
DROP TABLE IF EXISTS `student`;
CREATE TABLE IF NOT EXISTS `student` (
  `studno` int(4) NOT NULL,
  `name` varchar(10) NOT NULL,
  `id` varchar(20) NOT NULL,
  `grade` int(11) DEFAULT NULL CHECK (`grade` between 1 and 6),
...
CREATE TABLE IF NOT EXISTS `category` (
  ...
  `category_pic` varchar(50) NOT NULL,
  `열 4` enum('Y','N') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `member_email` varchar(250) NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `product` (
  ...
  `product_pic` varchar(200) DEFAULT 'defalut.jpg',
